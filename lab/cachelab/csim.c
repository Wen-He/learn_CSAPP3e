...
void Caching(cache_line *cache, operation an_op, int c_entry, int c_visible,summary *my_summary) {
    int index;
    int type;
    int result;
    ...
    cache_line *pcache = cache + an_op.index * c_entry;

    // get result type
    result = EVICTION;
    for (index = 0; index < c_entry; index++) {
    ...
    switch (result) {
        ...
        case HIT:
            ...
            for ( ; index < c_entry - 1 && (pcache + index + 1)->valid == 1; index++) {
        ...
        case EVICTION:
            ...
            for (index = 0; index < c_entry - 1; index++) {
                (pcache + index)->tag = (pcache + index + 1)->tag;
            }
            (pcache + index)->tag = an_op.tag;
            break;
...
int main(int argc, char **argv) {
    ...
    if (entry == 0 || block_power == 0 || trace == 0) {
        // ? can block_power be 0
    ...
    for (index = 0; index < set_num * entry; index++) {
        (cache + index)->valid = 0;
        // (cache + index)->tag = 0xffffffffffffffff;
    }
    ...
    while (fgets(line, 80, trace_file) != NULL) {
        ...
        Caching(cache, an_op, entry, visible,&my_summary);
    }
    free(cache);
